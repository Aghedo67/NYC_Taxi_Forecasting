...
with tab_fc:
    ...
    if forecasts is None:
        ...
        max_h = len(fdf)
        days_avail = sorted(fdf.ds.dt.date.unique())

        mode = st.radio("View", ["Next N hours", "Specific day"],
                        horizontal=True)
        if mode == "Next N hours":
            hours = st.slider("Hours ahead", min_value=6,
                              max_value=int(max_h),
                              value=min(72, int(max_h)), step=6)
            view = fdf.head(hours)
            window_label = f"next {hours}h"
        else:
            day = st.date_input(
                "Day to forecast", value=days_avail[0],
                min_value=days_avail[0], max_value=days_avail[-1],
                help=f"Forecast available {days_avail[0]} → {days_avail[-1]}")
            view = fdf[fdf.ds.dt.date == day]
            window_label = day.strftime("%a %d %b %Y")
